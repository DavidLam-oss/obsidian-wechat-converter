...
- source: user-confirmed
- scope: 贴图标题与正文字数计数、发布拦截
- state:
  - 标题 0–20 字、正文 0–1000 字保持普通计数颜色，达到上限仍可发布
  - 只有标题 21 字起、正文 1001 字起把当前数字标红并阻止发布
  - 标题必填仍由字段反馈和发布按钮表达，不把 `0/20` 计数误标为超限
- safety:
  - 弹窗按钮、发送动作与底层草稿服务都保留超限拦截
- aesthetic:
  - 红色只表示已经发生且会阻止发布的错误，使计数颜色与发布结果保持一致
