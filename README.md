...
2.  **🚀 一键同步到微信草稿箱 (v2.2 增强)**
    ...
    - **同步后自动清理（可选）**：支持在发送成功后自动清理封面图，默认关闭；可选“只删封面图”或“删整套封面图（整个封面文件夹）”。
    - **安全护栏**：只会删除你在设置里填写的“清理根目录”下的内容（默认留空，必须手动填写），并可优先移动到系统回收站。
...
- 清理目标：
  - **只删封面图（更安全）**：只删除封面图片
  - **删整套封面图（整个封面文件夹）**：删除封面所在文件夹
- 建议开启“使用系统回收站”，删除后还能找回
- 需先填写“清理根目录”（vault 相对路径），例如 `published`
