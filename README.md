...
2.  **🚀 一键同步到微信草稿箱 (v2.2 增强)**
    ...
    - **智能封面处理**：封面优先使用你在文档 frontmatter 里写好的 `cover`，没有就自动用正文第一张图，也支持手动上传覆盖。
    - **智能摘要辅助**：摘要优先使用 frontmatter 的 `excerpt`，没有就自动提取前 45 字，支持手动修改并带字数统计。
    - **同步后自动清理（可选）**：支持在发送成功后自动清理“设置里指定的目录”，默认关闭。
    - **支持占位符目录**：清理目录支持 `{{note}}`，可按当前文档名自动拼接目录，例如 `published/{{note}}_img`。
...
4. **一键同步到微信草稿箱** ⭐ 新功能
   ...
   - 点击 **[🚀 一键同步]** 按钮，选择账号和封面图
   - 如果文档 frontmatter 中有以下字段，会优先作为默认值：
     - `excerpt`：摘要
     - `cover`：封面图片路径（vault 相对路径）
     - `cover_dir`：封面目录路径（vault 相对路径）
   - 如果你手动上传封面或手动编辑摘要，会覆盖自动默认值
   - 文章将自动同步到微信公众号草稿箱

### 同步后自动清理（可选）

- 入口：插件设置 → 高级设置
- 默认关闭，开启后仅在“发送到微信草稿箱成功”后才会触发清理
- 清理对象：删除你在设置里填写的“清理目录”（目录删除，不是单文件删除）
- 清理目录支持 `{{note}}` 占位符，会自动替换为当前文档名
- 清理成功后，如果当前文档 frontmatter 里的 `cover` / `cover_dir` 指向被删目录，会自动清空这些失效字段
- 建议开启“使用系统回收站”，删除后还能找回
- 示例：`published/{{note}}_img`（当文档名为 `post` 时，实际删除 `published/post_img`）
