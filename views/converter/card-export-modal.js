...
- `startCardExport()`：按导出范围（全部 / 自选页）冻结合格快照 → 建任务 → 逐页输出 → 进度刷新；
...
- 导出范围由弹窗内的范围控件决定（`resolveCardExportScope`，实现在 card-export-modal-view）：
  默认 'all' 不带页 id；'selected' 带会话里的自选页，空选绝不改判成全部，
  直接不建任务，由表单的禁用态与清单说明兜底。
...
async startCardExport() {
  ...
  // 导出范围：'all' 不带页 id；'selected' 用弹窗清单里的自选页（空选兜底：不建任务，表单本已禁用按钮）
