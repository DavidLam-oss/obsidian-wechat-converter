...
- `services/card-*` 模块负责图片卡片导出全流程：属于纯本地客户端逻辑，**无任何新增后端服务、云端 API 或付费依赖**；单张复制经内存 Blob 写入系统剪贴板（零磁盘写入）；批量导出严格在 Vault 相对目录内独占写入（`cover.png` 与 `card-001.png` 等），具备符号链接/路径穿越安全校验与失败页清单契约。
