...
- `services/card-*` 模块负责图片卡片导出全流程：属于纯本地客户端逻辑，**无任何新增后端服务、云端 API 或付费依赖**；单张复制经内存 Blob 写入系统剪贴板（零磁盘写入）；批量导出严格在 Vault 相对目录内独占写入（`card-000.png`（封面）与 `card-001.png` 等），具备符号链接/路径穿越安全校验与失败页清单契约。
...
- **图片卡片导出为纯本地计算与 Vault 本地文件写入，不发起任何新增外部网络 API 请求**（远程图片抓取仅复用既有受控 HTTP(S) 策略经 Obsidian `requestUrl` 转发后内联为 `data:` URL）。

## 数据流

输入来自当前 Obsidian Markdown 文件、frontmatter、插件设置、本地图片资源和用户在发布弹窗中的临时选择。处理过程包括 Markdown 渲染、贴图纯文本语义降级、HTML 清洗、图片路径解析、媒体压缩/上传、数学公式转换、Mermaid 栅格化和发布 payload 组装。卡片模式输入经过 Markdown 语义分块、真实 DOM 测量与页计划装箱、资源内联快照构建，输出侧栏实时缩略图流；导出数据流经全局单捕获槽（有界排队）、逐页离屏重绘与 PNG 捕获，最终内存写入剪贴板或排他写入 Vault 批次目录（仅有失败页时输出 `export-manifest.json`）。最终输出包括预览 DOM、剪贴板富 HTML、微信草稿、飞书云文档、多平台草稿任务和本地卡片 PNG 文件。
